...
bool Mesh::intersectFace(const Ray& ray, Hit& hit, int faceId) const
{
    ...
    //Möller-Trumbore
    Point3f A = this->vertexOfFace(faceId,0).position;
    Vector3f nA = this->vertexOfFace(faceId,0).normal;
    Point3f B = this->vertexOfFace(faceId,1).position;
    Vector3f nB = this->vertexOfFace(faceId,1).normal;
    Point3f C = this->vertexOfFace(faceId,2).position;
    Vector3f nC = this->vertexOfFace(faceId,2).normal;
    ...
    float v = ray.direction.dot(Q) * det_inv;

    if(v < 0.f || u + v  > 1.f) {
      return false;
    }

    float t = e2.dot(Q) * det_inv;
